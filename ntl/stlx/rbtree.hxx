...
#pragma warning(default:4201)
        ...
        {
          links.left = n.links.left; links.right = n.links.right;
        }
      private:
    ...
    public:
      ...
      iterator erase(iterator position)
      {
        ...
        if(z->links.left == NULL || z->links.right == NULL){
          y = z;
        }else{
        ...
        node* x = y->link[ y->links.left == NULL ];
        ...
        if(x)
          x->parent = y->parent;
        if(y->parent)
          y->parent->link[ y != y->parent->links.left ] = x;
        else{
          prev_root = root_;
          root_ = x;
        }
        ...
        if(y != z){
          //z->data = y->data;
          // save z fields
          node* l[3] = {z->links.left, z->links.right, z->parent};
          int8_t c = y->color;
    ...
    protected:
      ...
      void fixup_delete(node* x) __ntl_nothrow
      {
        ...
        while(x != root_ && x->color == node::black){
          x = fixup_delete(x, x == x->parent->links.left);
        }
        x->color = node::black;
      }
