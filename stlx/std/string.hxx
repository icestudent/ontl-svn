...
public:
    ...
    {
      const size_t rlen = stlx::min(size(), str.size());
      if(!rlen)
    ...
    int compare(size_type pos, size_type n, const basic_string& str) const
    {
      if(pos+n >= size())
        return -1;
      const size_t rlen = stlx::min(n, str.size());
      if(!rlen)
    ...
    int compare(size_type pos1, size_type n1, const basic_string& str, size_type pos2, size_type n2) const
    {
      if(pos1+n1 >= size())
        ...
        return 1;
      const size_t rlen = stlx::min(n1, n2);
      if(!rlen)
...
template <> struct hash<stlx::string>: __::string_hash<stlx::string>{};
template <> struct hash<stlx::wstring>: __::string_hash<stlx::wstring>{};
template <> struct hash<stlx::u16string>: __::string_hash<stlx::u16string>{};
template <> struct hash<stlx::u32string>: __::string_hash<stlx::u32string>{};
