...
#ifndef STLX_TYPE_TRAITS_GNUC
#define STLX_TYPE_TRAITS_GNUC
...
private:
  template <class U>
  static __::sfinae_passed_tag test(int U::*)
  {
    return __::sfinae_passed_tag();
  }
  template <class>
  static __::sfinae_failed_tag test(...)
  {
    return __::sfinae_failed_tag();
  }
...
struct is_class_or_union
  : conditional<is_class_or_union_impl<T>::value, true_type, false_type>::type
{
};
...
template <
  class Base,
  class Derived,
  bool BothClassOrUnion,
  bool Same>
struct is_base_of_impl
{
private:
  typedef typename remove_cv<Base>::type     base_t;
  typedef typename remove_cv<Derived>::type  derived_t;
  ...
  template <class U>
  static __::sfinae_passed_tag test(derived_t&, U);
  static __::sfinae_failed_tag test(base_t&, int);

public:
  static const bool value = NTL_SFINAE_EVAL(test(testee_t(), 0));
};
template <
  class Base,
  class Derived,
...
template <class Base, class Derived> struct is_base_of
  : public integral_constant<
    ...
    __::is_base_of_impl<
      Base,
      Derived,
      __::is_class_or_union<Base>::value && __::is_class_or_union<Derived>::value,
      is_same<Base, Derived>::value>::value
      > {};
...
#pragma warning(disable:4820) // 'X' bytes padding added after data member 'stlx::aligned_storage<Len,Align>::type::data'

namespace aux {
template <stlx::size_t Align>struct aligner;
...
#ifdef NTL__CXX_VT
template <stlx::size_t Len, class... Types> struct aligned_union;
#endif
...
STLX_DEF_TRAIT2(has_trivial_default_constructor, has_trivial_constructor)

STLX_DEF_TRAIT2(has_trivial_copy_constructor, has_trivial_copy)

STLX_DEF_TRAIT(has_trivial_assign)

STLX_DEF_TRAIT(has_trivial_destructor)

STLX_DEF_TRAIT2(has_nothrow_default_constructor, has_nothrow_constructor)

STLX_DEF_TRAIT2(has_nothrow_copy_constructor, has_nothrow_copy)
...
#endif//#ifndef STLX_TYPE_TRAITS_GNUC
