...
/// supports unique keys (contains at most one of each key value)
  ...
  namespace __
  {
    ...
    friend class stlx::map<Key, T, Compare, Allocator>;
    protected:
      Compare comp;
...
public:
    ...
    {
      return tree_type::insert(stlx::forward<P>(x));
    }

    //template <class P>
    //iterator insert(const_iterator position, P&& x);
    //{
    //  return tree_type::insert(position, stlx::forward<P>(x)).first;
    //}
#endif
    ...
    {
      tree_type::swap(x);
      stlx::swap(val_comp_, x.val_comp_);
    }
