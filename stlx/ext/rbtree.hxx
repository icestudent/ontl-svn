...
namespace tree
{
  namespace rb_tree
  {
    ...
    protected:
      ...
      {
        node_type* const np = node_allocator.allocate(1);
        node_allocator.construct(np, stlx::forward<value_type>(x));
        return np;
      }

      stlx::pair<iterator, bool> insert_reference(value_type&& x)
      {
        return insert_impl(construct_node(stlx::forward<value_type>(x)));
      }
      #endif

      stlx::pair<iterator, bool> insert_impl(node* const np)
      {
        // I guess this is not necessary:
        // Node_allocator.allocate shall throw bad_alloc in this case.
        // Moreover logic_error contains stlx::string member which
        // potentially could not be allocated as well. - ST
        // if(count_ == max_size()-1)
        // __ntl_throw(stlx::length_error("rb_tree<T> too long"));
        ...
        if(empty()){
          ...
          first_ = last_ = root_;
          ++count_;
          return stlx::make_pair(iterator(root_, this), true);
        }
        ...
        for(node* p = root_; p != NULL; q = p, p = p->u.link[ greater ]){
          ...
          if(!greater && !elem_less(np->elem, p->elem))
            // equal
            return stlx::make_pair(iterator(p, this), false);
        }
        ...
        // balance tree
        fixup_insert(np);
        return stlx::make_pair(iterator(np, this), true);
      }

    public:
      stlx::pair<iterator, bool> insert(const value_type& x)
      {
      ...
      #endif
      {
        if(this != &tree){
          ...
          stlx::swap(root_, tree.root_);
          stlx::swap(count_, tree.count_);
          stlx::swap(comparator_, tree.comparator_);
          stlx::swap(node_allocator, tree.node_allocator);
        }
      }
    ...
    bool operator != (const rb_tree<T, Compare, Allocator>& x, const rb_tree<T, Compare, Allocator>& y)
    {
      return stlx::rel_ops::operator !=(x, y);
    }
